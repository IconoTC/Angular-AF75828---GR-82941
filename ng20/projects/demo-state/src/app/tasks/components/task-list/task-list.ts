...
@Component({
  ...
  providers: [{ provide: InMemoryRepo, useClass: InMemoryRepo }],
  template: `
    <details #addTaskDetails>
      <summary>Add Task</summary>
      <ind-task-add (eventAdd)="handleAdd($event)" />
    </details>

    <details #addTaskDetails2>
      <summary>Add Task Form</summary>
      <ind-task-form [parentElement]="element2().nativeElement" (eventAdd)="handleAdd($event)" />
    </details>
